...
LOGS_DIR=../../logs/medprompt/cot
DATA_DIR=../../data
OUTPUT_DIR=../../outputs/medprompt/cot
...
for model in gpt-4o-1120-nofilter-global; do
    {
        for dataset in medqa pubmedqa medmcqa medbullets mmlu mmlu-pro medexqa medxpertqa-r medxpertqa-u; do
            ...
            for split in test_hard; do
                ...
                python cot.py --dataset_name $dataset --dataset_dir $DATA_DIR/$dataset/ --split $split --model $model --output_files_folder $OUTPUT_DIR --num_processes 4 | tee $log_file
                # 2> $error_file
            done
        done
    } &
