...
def run_few_shot(question_text: str, options_text: str, client: Any, model: str, few_shot_examples: List[Dict]) -> tuple[Any, str]:
    ...
    if model in ["o1-mini", "o3-mini", "claude-3-5-sonnet", "claude-3-5-haiku"]:
        ...
        messages = [
            {
                "role": "system", 
                "content": "You are a knowledgeable medical assistant. Provide accurate answers to the medical question based on the given information. The following are multiple choice questions (with answers) about medical knowledge."
            },
            {
                "role": "user",
                "content": (
...
def run(problem: Dict, client: Any, model: str = "o3-mini", few_shot_examples: List[Dict] = [], retries: int = 3) -> Dict:
    ...
    for attempt in range(retries):
        try:
            ...
            completion, raw_response = run_few_shot(question_text, options_text, client, model, few_shot_examples)
...
if __name__ == "__main__":
    ...
    existing_output_file = os.path.join(args.output_files_folder, args.dataset_name, f"{args.model_name.split('/')[-1]}-{args.dataset_name}-{args.split}-few_shot.json")
    ...
    few_shot_examples = load_jsonl(os.path.join(args.dataset_dir, f"train.jsonl"))[:5]
    ...
    with ThreadPoolExecutor(max_workers=args.num_processes) as executor:
        futures = {executor.submit(run, problem, client, args.model_name, few_shot_examples): problem for problem in problems_to_process}
