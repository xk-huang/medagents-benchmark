...
for model in gpt-4o-mini gpt-4o deepseek-ai/DeepSeek-V3; do
    {
        for dataset in medexqa medxpertqa-r medxpertqa-u; do
            ...
            for split in test_hard; do
                ...
                python self_refine.py --dataset_name $dataset --dataset_dir $DATA_DIR/$dataset/ --split $split --model $model --output_files_folder ../../output/ --num_processes 4 --num_rounds 3 > $log_file 2> $error_file
            done
        done
    } &
done
