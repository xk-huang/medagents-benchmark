...
for model in gpt-4o-1120-nofilter-global; do
    {
        for dataset in medqa medbullets medmcqa pubmedqa mmlu mmlu-pro medexqa medxpertqa-r medxpertqa-u; do
            ...
            for split in test_hard; do
                ...
                python medprompt.py --dataset_name $dataset --dataset_dir $DATA_DIR/$dataset/ --split $split --model $model --output_files_folder $OUTPUT_DIR --num_processes 4 --num_rounds 3 --vote_count 3 | tee $log_file
                # 2> $error_file
            done
        done
    } &
